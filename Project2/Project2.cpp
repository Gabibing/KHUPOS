...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		{
			...
			Pos::getInstance().initialize(hWnd);
			DB::getInstance().logging(TEXT("프로그램 실행"));
	...
	case WM_TIMER:
		...
		{
		case 1:
			Pos::getInstance().render();
			Pos::getInstance().update();
			break;
		}
		break;
	case WM_CHAR:
		{
			Pos::getInstance().keydown((WCHAR)wParam);
		}
		break;
	case WM_SETCURSOR:
		{
    ...
    case WM_PAINT:
        ...
        break;
    case WM_DESTROY:
		DB::getInstance().logging(L"프로그램 종료");
        PostQuitMessage(0);
        break;
    default:
        return DefWindowProc(hWnd, message, wParam, lParam);
